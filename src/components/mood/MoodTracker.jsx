  ...
  return (
    <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
      ...
      {[0, 45, 90, 135, 180, 225, 270, 315].map((a, i) => {
        ...
        const x1 = 18 + Math.cos(rad) * 9.5
        const y1 = 18 + Math.sin(rad) * 9.5
        const x2 = 18 + Math.cos(rad) * 12
        const y2 = 18 + Math.sin(rad) * 12
        return (
          <line key={a} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#D4A853" strokeWidth="1.5" strokeLinecap="round">
...
function FujiIcon() {
  return (
    <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
      ...
      {/* Sunrise glow */}
      <ellipse cx="18" cy="26" rx="14" ry="4" fill="url(#sunrise)" opacity="0.6"/>
      {/* Mountain */}
...
function MoodCard({ mood, isSelected, onSelect, isLocked }) {
  const Icon = MOOD_ICONS[mood.level]
  return (
    <motion.button
      onClick={() => !isLocked && onSelect(mood.level)}
      whileTap={!isLocked ? { scale: 0.98 } : {}}
      animate={isSelected ? { scale: 1.02 } : { scale: 1 }}
      transition={{ duration: 0.15 }}
      disabled={isLocked}
      className="relative w-full flex items-center gap-3 text-left transition-all"
      style={{
      ...
      <div className="flex flex-col gap-2">
        {MOODS.map(mood => (
          <MoodCard
            key={mood.level}
            mood={mood}
            isSelected={todayMood === mood.level}
